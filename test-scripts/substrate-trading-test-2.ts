...
process.nextTick(async () => {
  ...
  const rpcTradingGetPositions = await rpcAsAny.trading.get_positions(
    ...
    // convertHexToU256('0x01234'),
    '0x445448065fa6ea97a3918dcd99756ae0c0e9af0e0013826c30478f8378a1bdaf',
    '1431520323',
  );


  console.log({ rpcTradingGetPositions: rpcTradingGetPositions.toPrimitive() });

  const rpcTradingGetPositionsResolved = rpcTradingGetPositions.toPrimitive();

  console.log({ direction: rpcTradingGetPositionsResolved[0].direction });
  ...
  console.log({ collateralAssetId: usdcAsset.id.toString(), ethAssetId: ethAsset.id.toString() });
  ...
  for await (const account of [account1, account2]) {
    ...
    console.log({ index: account.index, accountId : account.id });
