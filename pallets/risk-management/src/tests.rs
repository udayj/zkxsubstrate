	...
	env.execute_with(|| {
		...
		let _ = TradingAccounts::set_balances(
			...
			vec![BalanceUpdate {
				...
				balance_value: FixedI128::from_float(-1.0),
			}],
		);
		...
		assert!(
			insurance_fund_balance_after ==
				insurance_fund_balance_before - FixedI128::from_inner(10001500000000000000000),
			"Invalid balance of insurance fund"
		);
		...
		assert_has_events(vec![
			Event::InsuranceFundChangeV2 {
				market_id,
				amount: FixedI128::from_inner(10001500000000000000000),
				modify_type: FundModifyType::Decrease,
				block_number: 1,
			}
			.into(),
