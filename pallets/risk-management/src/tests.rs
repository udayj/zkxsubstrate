...
#[test]
fn test_liquidation_underwater_negative_balance() {
	let mut env = setup();
	let market_id: u128 = btc_usdc().market.id;
	let btc_insurance_fund = U256::from(2_u8);
	let collateral_id = usdc().asset.id;
	...
	env.execute_with(|| {
		...
		let index_price1 = MultiplePrices {
			market_id,
			index_price: FixedI128::from_float(7999.7),
			mark_price: FixedI128::from_float(7999.7),
		};
		index_prices.push(index_price1);
		assert_ok!(Prices::update_prices(
			RuntimeOrigin::signed(sp_core::sr25519::Public::from_raw([1u8; 32])),
		...
		// reduce the user's balance below 0
		let new_user_balance = FixedI128::from_float(-1.0);
		let _ = TradingAccounts::set_balances(
			RuntimeOrigin::signed(sp_core::sr25519::Public::from_raw([1u8; 32])),
			alice_id,
			vec![BalanceUpdate { asset_id: collateral_id, balance_value: new_user_balance }],
		);

		// Place Forced order for liquidation
		let charlie_order = Order::new(204.into(), charlie_id)
			.set_size(5.into())
			.set_price(FixedI128::from_float(7999.7))
			.set_leverage(5.into())
			.sign_order(get_private_key(charlie().pub_key));
		...
		// Margin + PNL = (-1.5) < 0
		// Balance: -1
		// Case: Balance negative; Margin + pnl negative
		let pnl = FixedI128::from_inner(-10001500000000000000000);
		let insurance_fund_balance_after =
			TradingAccounts::insurance_fund_balance(btc_insurance_fund, collateral_id);
		assert!(
			insurance_fund_balance_after == insurance_fund_balance_before + pnl,
			"Invalid balance of insurance fund"
		);
		...
		assert_eq!(TradingAccounts::balances(alice_id, collateral_id), new_user_balance + pnl);
...
#[test]
fn test_liquidation_underwater_non_negative_balance() {
	let mut env = setup();
	let market_id: u128 = btc_usdc().market.id;
	let btc_insurance_fund = U256::from(2_u8);
	let collateral_id = usdc().asset.id;
	...
	env.execute_with(|| {
		...
		let margin = TradingAccounts::get_margin_info(
			alice_id,
			collateral_id,
			FixedI128::zero(),
			FixedI128::zero(),
		);
		print!("User margin: {:?}", margin);
		...
		// Margin + PNL = (-1.5) < 0
		// Balance: 10000
		// Case: Balance positive; Margin + pnl negative
		let pnl = FixedI128::from_inner(-10001500000000000000000);
		let user_balance = FixedI128::from_u32(10000);
		let insurance_fund_change = pnl + user_balance;
		print!(
			"insurance fund: {:?}:{:?}",
			insurance_fund_change,
			insurance_fund_balance_before + insurance_fund_change
		);
		let insurance_fund_balance_after =
			TradingAccounts::insurance_fund_balance(btc_insurance_fund, collateral_id);
		assert!(
			insurance_fund_balance_after == insurance_fund_balance_before + insurance_fund_change,
			"Invalid balance of insurance fund"
		);
		...
		assert_eq!(TradingAccounts::balances(alice_id, collateral_id), user_balance + pnl);

		assert_has_events(vec![
			Event::InsuranceFundChangeV2 {
				market_id,
				amount: -insurance_fund_change,
				modify_type: FundModifyType::Decrease,
				block_number: 1,
			}
			.into(),
...
#[test]
fn test_liquidation_underwater_sufficient_balance() {
	let mut env = setup();
	let market_id: u128 = btc_usdc().market.id;
	let btc_insurance_fund = U256::from(2_u8);
	let collateral_id = usdc().asset.id;
	...
	env.execute_with(|| {
		...
		let new_user_balance = 10002.into();
		let _ = TradingAccounts::set_balances(
			RuntimeOrigin::signed(sp_core::sr25519::Public::from_raw([1u8; 32])),
			alice_id,
			vec![BalanceUpdate { asset_id: collateral_id, balance_value: new_user_balance }],
		...
		let insurance_fund_balance_after =
			TradingAccounts::insurance_fund_balance(btc_insurance_fund, collateral_id);
		assert!(
			insurance_fund_balance_after == insurance_fund_balance_before,
			"Invalid balance of insurance fund"
		);
