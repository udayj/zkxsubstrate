...
use pallet_support::{
	...
	types::{
		...
		Position, Side, TradingAccountMinimal,
	},
};
...
fn test_liquidation() {
	...
	env.execute_with(|| {
		...
		let alice_id: U256 = get_trading_account_id(TradingAccountMinimal {
			account_address: U256::from_dec_str(
				"514985946304409918108156904474323488680576689233017441348698245025232394431",
			)
			.unwrap(),
			pub_key: U256::from_dec_str(
				"2115094115460180822149397430584036713549113656006837840284277481087320154189",
			)
			.unwrap(),
			index: 0,
		});

		print!("User account is {:?}", alice_id);
	});
}
