...
use zkx_support::test_helpers::accounts_helper::get_trading_account_id;
...
fn sign_order(order: Order, private_key: U256) -> Order {
	let order_hash = order.hash(&order.signature_info.hash_type).unwrap();
	...
	let signature_info = SignatureInfo { sig_r, sig_s, ..order.signature_info };
	Order { signature_info, ..order }
}
...
fn it_works_for_open_trade_simple() {
	new_test_ext().execute_with(|| {
		...
		let account_id_1: U256 = get_trading_account_id(accounts[0]);
		let account_id_2: U256 = get_trading_account_id(accounts[1]);
